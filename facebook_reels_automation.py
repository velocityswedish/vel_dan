"""
Facebook Reels Automation - Bilingual English/Danish Content Generator
IMPROVED VERSION: Better backgrounds, English categories, no repeats, Velocity Danish branding
"""
...
# Danish translations for display
CATEGORIES_DANISH = {
...
def generate_phrases(category_english: str, num_phrases: int = 5) -> list:
    ...
    for attempt in range(max_attempts):
        try:
            ...
            prompt = f"""Create {num_phrases * 2} unique {category_english} phrases for English speakers learning Danish.
...
2. Danish translation (with commas matching the rhythm)
...
IMPORTANT: Create FRESH, UNIQUE phrases that haven't been used before."""
            ...
            payload = {
                "model": AI_MODEL,
                "messages": [
                    {"role": "system", "content": "You are a Danish teacher. Create short, natural phrases with pauses."},
...
def get_fresh_fallback_phrases(category: str, num_phrases: int) -> list:
    ...
    all_fallbacks = {
        "Motivation": [
            {"english": "Believe in yourself.", "danish": "Tro på dig selv.", "pronunciation": "Tro på dy sel."},
            {"english": "You are capable of amazing things.", "danish": "Du er i stand til utrolige ting.", "pronunciation": "Doo er ee stand til oo-tro-li-ge ting."},
            {"english": "Dream big, start small.", "danish": "Drøm stort, start småt.", "pronunciation": "Druhm stort, start smowt."},
            {"english": "Your future is created by your actions.", "danish": "Din fremtid skabes af dine handlinger.", "pronunciation": "Deen frem-teed skah-bes af dee-ne hand-lin-ger."},
            {"english": "Never give up on your dreams.", "danish": "Giv aldrig op på dine drømme.", "pronunciation": "Giv al-dree op på dee-ne druhm-me."},
        ],
        "Love": [
            {"english": "Love yourself first.", "danish": "Elsk dig selv først.", "pronunciation": "Elsk dy sel furst."},
            {"english": "Love makes everything possible.", "danish": "Kærlighed gør alt muligt.", "pronunciation": "Ker-lihed gur alt muh-light."},
        ],
    }
...
def generate_all_audio(phrases: list, output_dir: str):
    ...
    for i, phrase in enumerate(phrases):
        ...
        # Generate Danish audio
        da_success = asyncio.run(generate_single_audio(phrase["danish"], DANISH_VOICE, str(danish_file)))
        if da_success:
            print(f"    ✓ Danish: {danish_file.name}")
        else:
            cmd = ["ffmpeg", "-y", "-f", "lavfi", "-i", "anullsrc=r=24000:cl=mono", "-t", "2", str(danish_file)]
            subprocess.run(cmd, capture_output=True)

        # Get ACTUAL durations
        en_duration = get_audio_duration(str(english_file))
        da_duration = get_audio_duration(str(danish_file))

        # Add pause between English and Danish
        pause_between = 0.5
        total_duration = en_duration + pause_between + da_duration

        print(f"    ⏱️  Total: {total_duration:.2f}s (EN: {en_duration:.2f}s + pause: {pause_between}s + DA: {da_duration:.2f}s)")
        ...
        if result.returncode != 0:
            ...
            with open(concat_file, "w", encoding="utf-8") as f:
                ...
                f.write(f"file '{danish_file.as_posix()}'\n")
        ...
        audio_files.append({
            ...
            "danish": str(danish_file),
            "combined": str(combined_file),
            "duration": actual_duration,
            "en_duration": en_duration,
            "da_duration": da_duration
        })
...
def generate_complete_image(phrase_data: dict, category_english: str, output_path: str):
    ...
    # Danish text
    danish_y = english_y + total_height + 110  # Increased from 100
    danish_lines = wrap_text(danish, font_large, VIDEO_WIDTH - 140)
    total_height = len(danish_lines) * 95  # Increased from 75

    draw.rectangle(
        [(60, danish_y - 55), (VIDEO_WIDTH - 60, danish_y + total_height + 15)],
        fill=(80, 30, 30, 220)
    )

    for i, line in enumerate(danish_lines):
        y_pos = danish_y + (i * 95)  # Increased spacing
        draw.text(
            (VIDEO_WIDTH // 2, y_pos),
            line,
            fill=(255, 255, 0),
    ...
    pronunciation_y = danish_y + total_height + 90  # Increased from 80
...
def generate_reel(category_english: str = None):
    ...
    print(f"\n{'='*80}")
    print(f"Category: {category_english} ({CATEGORIES_DANISH[category_english]})")
    print(f"{'='*80}\n")
    ...
    print("\n[3/4] Generating audio (English + Danish with 500ms pause)...")
    ...
    print(f"  🏷️  Branding: Velocity Danish")
    print(f"{'='*80}\n")
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print("🇩🇰 VELOCITY DANISH - FACEBOOK REELS AUTOMATION 🇩🇰")
    print("="*80)
    print("\n✨ IMPROVED FEATURES:")
    ...
    print("  ✓ Velocity Danish branding at bottom")
