...
def generate_video_metadata(category: str, num_phrases: int):
    """Generate Danish title, description, and tags for the video"""

    title = f"Dansk Læring: {num_phrases} Essentielle {category} Sætninger 🇩🇰"

    description = f"""Lær {num_phrases} essentielle danske sætninger for {category}!

Perfekt for begyndere, der lærer dansk. Hver sætning indeholder:
✅ Engelsk sætning
✅ Dansk oversættelse
✅ Udtaleguide
✅ Brugskontekst

Abonner for daglige dansktimer!

#DanskLæring #DanskTimer #DanskForBegyndere #SprogLæring
#Dansk #Uddannelse #Tutorial #DagligDansk #{category.replace(' ', '')}
"""
...
def upload_to_youtube(video_path, title, description, tags=None, category_id='27'):
    ...
    if tags is None:
        tags = ['dansk læring', 'danish lessons']
...
def main():
    ...
    # Generate Danish metadata for test
    title = "Dansk Læring: 5 Essentielle Motivation Sætninger 🇩🇰"
    description = "Lær 5 essentielle danske sætninger for Motivation! #DanskLæring #DanskTimer"
    tags = ['dansk læring', 'danish lessons', 'motivation']
